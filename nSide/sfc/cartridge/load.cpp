...
auto Cartridge::loadBSMemory(Markup::Node node) -> void {
  ...
  loadMemory(bsmemory.memory, node["board/rom"], false, ID::BSMemory);
}

auto Cartridge::loadSufamiTurboA(Markup::Node node) -> void {
  information.title.sufamiTurboA = node["information/title"].text();

  loadMemory(sufamiturboA.rom, node["board/rom"], false, ID::SufamiTurboA);
  loadMemory(sufamiturboA.ram, node["board/ram"], true, ID::SufamiTurboA);
...
auto Cartridge::loadSufamiTurboB(Markup::Node node) -> void {
  ...
  loadMemory(sufamiturboB.rom, node["board/rom"], false, ID::SufamiTurboB);
  loadMemory(sufamiturboB.ram, node["board/ram"], true, ID::SufamiTurboB);
}
...
auto Cartridge::loadSufamiTurbo(Markup::Node root, bool slot) -> void {
  ...
  if(slot == 0) interface->loadRequest(ID::SufamiTurboA, "Sufami Turbo", "st", false);
