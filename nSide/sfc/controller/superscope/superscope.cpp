...
SuperScope::SuperScope(bool port, uint index) : Controller(port, index) {
...
auto SuperScope::data() -> uint2 {
  ...
  if(counter == 0) {
    ...
    bool newturbo = interface->inputPoll(port, index, Turbo);
    ...
    cursor = interface->inputPoll(port, index, Cursor);

    //pause is a button; it is always edge sensitive
    pause = false;
    bool newpause = interface->inputPoll(port, index, Pause);
