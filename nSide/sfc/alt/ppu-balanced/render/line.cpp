...
inline auto PPU::get_direct_color(uint8_t p, uint8_t t) -> uint16_t {
  return ((t & 7) << 2) | ((p & 1) << 1) |
    (((t >> 3) & 7) << 7) | (((p >> 1) & 1) << 6) |
    ((t >> 6) << 13) | ((p >> 2) << 12);
}

inline auto PPU::get_pixel_normal(uint32_t x) -> uint16_t {
...
inline auto PPU::get_pixel_swap(uint32_t x) -> uint16_t {
  pixel_t& p = pixel_cache[x];
  uint16_t src_main, src_sub;
  uint8_t bg_sub;
...
inline auto PPU::render_line_clear() -> void {
  auto ptr = (uint32_t*)output + (line * 1024) + ((interlace() && field()) ? 512 : 0);
  memory::fill(ptr, 512 * sizeof(uint32_t));
}
