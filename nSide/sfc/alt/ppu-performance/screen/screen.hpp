class Screen {
  ...
  alwaysinline auto addsub(uint x, uint y, bool halve) -> uint16_t;
  auto scanline() -> void;
  auto render_black() -> void;
  alwaysinline auto get_pixel_main(uint x) -> uint16_t;
  alwaysinline auto get_pixel_sub(uint x) -> uint16_t;
  auto render() -> void;
