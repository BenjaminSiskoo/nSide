auto CPU::dma_transfer_valid(uint8_t bbus, uint abus) -> bool {
...
auto CPU::dma_read(uint abus) -> uint8_t {
  if(dma_addr_valid(abus) == false) return 0x00;
  return bus.read(abus, regs.mdr);
}

auto CPU::dma_write(bool valid, uint addr, uint8_t data) -> void {
...
auto CPU::dma_bbus(uint i, uint index) -> uint8_t {
