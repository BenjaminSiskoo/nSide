auto CPU::dma_transfer_valid(uint8_t bbus, uint_t abus) -> bool {
...
auto CPU::dma_addr_valid(uint_t abus) -> bool {
...
auto CPU::dma_read(uint_t abus) -> uint8_t {
  if(dma_addr_valid(abus) == false) return 0x00;
  return bus.read(abus, regs.mdr);
}

auto CPU::dma_write(bool valid, uint_t addr, uint8_t data) -> void {
  if(valid) bus.write(addr, data);
}

auto CPU::dma_transfer(bool direction, uint8_t bbus, uint_t abus) -> void {
  if(direction == 0) {
    uint8_t data = dma_read(abus);
    add_clocks(8);
    dma_write(dma_transfer_valid(bbus, abus), 0x2100 | bbus, data);
  } else {
    uint8_t data = dma_transfer_valid(bbus, abus) ? bus.read(0x2100 | bbus, regs.mdr) : 0x00;
    add_clocks(8);
    dma_write(dma_addr_valid(abus), abus, data);
  }
}

auto CPU::dma_bbus(uint_t i, uint_t index) -> uint8_t {
...
auto CPU::dma_addr(uint_t i) -> uint_t {
  uint_t result = (channel[i].source_bank << 16) | (channel[i].source_addr);
...
auto CPU::hdma_addr(uint_t i) -> uint_t {
  return (channel[i].source_bank << 16) | (channel[i].hdma_addr++);
}

auto CPU::hdma_iaddr(uint_t i) -> uint_t {
...
auto CPU::hdma_active_after(uint_t i) -> bool {
  for(uint_t n = i + 1; i < 8; i++) {
    if(channel[i].hdma_enabled && !channel[i].hdma_completed) return true;
  }
  return false;
}

auto CPU::hdma_update(uint_t i) -> void {
...
auto CPU::hdma_run() -> void {
  ...
  for(uint_t i = 0; i < 8; i++) {
    ...
    if(channel[i].hdma_do_transfer) {
      static const uint_t transfer_length[] = {1, 2, 2, 4, 4, 4, 2, 4};
      uint_t length = transfer_length[channel[i].transfer_mode];
      for(uint_t index = 0; index < length; index++) {
        uint_t addr = channel[i].indirect == false ? hdma_addr(i) : hdma_iaddr(i);
