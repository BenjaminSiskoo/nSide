...
inline auto PPU::renderLine_output() -> void {
  ...
  if(!io.pseudoHires && io.bgMode != 5 && io.bgMode != 6) {
    for(uint x : range(256)) {
      uint color = (io.displayBrightness << 15) | get_pixel_normal(x);
      *lineA++ = *lineB++ = color;
      *lineA++ = *lineB++ = color;
    }
  } else {
    for(uint x : range(256)) {
      *lineA++ = *lineB++ = (io.displayBrightness << 15) | get_pixel_swap(x);
      *lineA++ = *lineB++ = (io.displayBrightness << 15) | get_pixel_normal(x);
    }
  }
}
