...
auto WonderSwanColorInterface::videoSize(uint width, uint height, bool arc, bool intScale) -> VideoSize {
  uint w = 224;
  uint h = 224;
  double m;
...
auto WonderSwanColorInterface::connect(uint port, uint device) -> void {
...
auto WonderSwanColorInterface::rotate() -> void {
  system.rotate();
