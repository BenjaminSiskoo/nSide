auto CartPal::bsMemoryManifest(string location) -> string {
  vector<uint8_t> buffer;
  concatenate(buffer, {location, "program.rom"});
  return bsMemoryManifest(buffer, location);
}

auto CartPal::bsMemoryManifest(vector<uint8_t>& buffer, string location) -> string {
  string markup;
...
auto CartPal::bsMemoryImport(vector<uint8_t>& buffer, string location) -> string {
