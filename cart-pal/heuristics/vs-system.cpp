struct VSSystemCartridge {
  VSSystemCartridge(string location, uint8* data, uint size);
...
VSSystemCartridge::VSSystemCartridge(string location, uint8* data, uint size) {
  if(size < 16) return;
  if(data[0] != 'N') return;
  if(data[1] != 'E') return;
  if(data[2] != 'S') return;
  ...
  manifest.append("  ppu revision=", ppu_revision, "\n");
  manifest.append("  controller port=2 device=gamepad\n");
  manifest.append("  controller port=1 device=gamepad\n");
  ...
  case   1: // SxROM-like
    manifest.append("  chip type=MMC1B2\n");
    break;

  case   2: // UxROM-like
    manifest.append("  chip type=74HC32\n");
    break;

  case 206: // NAMCOT-34xx-like
    manifest.append("  chip type=108\n");
    break;
  }
